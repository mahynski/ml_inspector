...
class InspectModel:
    ...
    @staticmethod
    def learning_curve(
        self, model, X, y, train_sizes=np.linspace(0.1, 1, 10), cv=10
    ):
        """
    ...
    def kernelSHAP(
        ...
        """
        Perform KernelSHAP to explain a model.

        Kernel SHAP (SHapley Additive exPlanations) is a way of estimating
        Shapley values using regression.

        Shapley values themselves are feature importances for linear models in
        the presence of multicollinearity. The importance of a feature is
        computed by adding to all subsets of coalitions of other features; when
        one or more collinear features are present, a features impact would be
        small, but there are lots of subsets where those collinear features are
        absent from the initial coalition, thus capturing (to some extent) this
        effect.

        As in PFI, it is best to try to remove correlated features first using
        hierarchical clustering; this will make things easier computationally
        anyway.

        ``The fast computation makes it possible to compute the many Shapley
        values needed for the global model interpretations. The global
        ...
        * https://christophm.github.io/interpretable-ml-book/shap.html
        ...
        since it computes Shapley values.
        ...
        ... train_test_split(*shap.datasets.iris(), test_size=0.2,
        ... random_state=0)
        >>> model = sklearn.svm.SVC(kernel='rbf', probability=probability)
        >>> model.fit(X_train, Y_train)
        >>> explainer, shap_values = InspectModel.kernelSHAP(model, X_train,
        ... X_test, use_probabilities=probability, link=link)
        >>> # visualize the first prediction's explanation (use matplotlib=True
        ... to avoid Javascript, but JS is more interactive)
        >>> shap.force_plot(explainer.expected_value[0], shap_values[0][0,:],
        ... X_test.iloc[0,:], # Just 1 instance for first (Setosa) class
        ... link=link, matplotlib=False)
        >>> shap.force_plot(explainer.expected_value[0], shap_values[0],
        ... X_test,
        ... link=link) # All instances for first class, rotated and stacked
        ...
        >>> model.fit(X, y)
        >>> explainer, shap_values = InspectModel.kernelSHAP(model, X, X,
        ... nsamples=100)
        >>> # visualize the first prediction's explanation (use matplotlib=True
        ... to avoid Javascript, but JS is more interactive)
        >>> shap.force_plot(explainer.expected_value, shap_values[0,:],
        ... X.iloc[0,:]) # Just 1 instance
        >>> shap.force_plot(explainer.expected_value, shap_values, X) # All
        ... instances, rotated 90 degrees and stacked
        ...
        l1_reg : float
            ...
            values.
        link : str
            Link function to match feature importance values to the model
            output.  See ``shap.KernelExplainer``. Generally best to use
            'logit' when use_probabilities=True, and 'identity' when
            use_probabilities=False; however, these examples do not.
            (1) https://slundberg.github.io/shap/notebooks/ \
            Iris%20classification%20with%20scikit-learn.html,
            (2)  https://slundberg.github.io/shap/notebooks/ \
            Census%20income%20classification%20with%20scikit-learn.html
        k_means : int
            If > 0, use KMeans to summarize the training dataset which can
            greatly accelerate the calculation at the cost of accuracy.  This
        ...
        explainer = shap.KernelExplainer(
            model=(model.predict_proba if use_probabilities else model.predict),
            data=X_train,
            link=link,
        )
        shap_values = explainer.shap_values(
            X_test, nsamples=nsamples, l1_reg=l1_reg
        )
    ...
    @staticmethod
    def treeSHAP(
        model, X_train, X_test, approximate=False, check_additivity=True
    ):
        """
        Perform TreeSHAP to explain a model.

        A specialized (faster) implementation of kernelSHAP for tree-based
        models that is EXACT, not an approximation, of the SHapley values.

        See Lundberg et al. "From local explanations to global understanding
        with explainable AI for trees" Nat. Mach. Intell. (2020)
        ...
        >>> model = sklearn.ensemble.RandomForestRegressor()
        >>> model.fit(X, y)
        >>> explainer, shap_values, interaction_values =
        ... InspectModel.treeSHAP(model, X, X)
        >>> # visualize the first prediction's explanation (use
        ... matplotlib=True to avoid Javascript, but JS is more interactive)
        >>> shap.force_plot(explainer.expected_value, shap_values[0,:],
        ... X.iloc[0,:]) # Just 1 instance
        >>> shap.force_plot(explainer.expected_value, shap_values, X) # All
        ... instances, rotated 90 degrees and stacked
        ...
        explainer = shap.TreeExplainer(
            ...
            # shap_interaction_values only supported for this option currently
        )
        shap_values = explainer.shap_values(
            X_test, check_additivity=check_additivity, approximate=approximate
        )
    ...
    @staticmethod
    def deepSHAP():
        """Perform DeepSHAP to explain a model."""
